...
async function fetchTransaksi() {
  ...
  const cabangId = auth.user?.cabang_id
  try {
    const res = await getTransaksi(cabangId)
...
function exportExcel() {
  ...
  XLSX.writeFile(wb, `transaksi_${cabangId}.xlsx`)
}

onMounted(async () => {
  const cabangId = auth.user?.cabang_id
  if (!cabangId) {
    error.value = 'Data cabang tidak ditemukan'
    loading.value = false
    return
  }

  if (route.query.tab === 'penjualan' || route.query.tab === 'transaksi') {
    tab.value = route.query.tab
  }
  try {
    const res = await getSemuaCabang()
    const found = (res.data || []).find((b) => b.cabang_id === cabangId)
    branch.value = found || null
    if (branch.value) {
      if (tab.value === 'penjualan' || tab.value === 'transaksi') {
        fetchTransaksi()
      }
      try {
        const [p, pr] = await Promise.all([
          getPrediksi(cabangId),
          getPromo(cabangId),
        ])
        prediksi.value = p
        promo.value = pr
      ...
      try {
        engineering.value = await getEngineering(cabangId)
      } catch (e) {
...
<template>
  <div>
    <div v-if="!auth.user?.cabang_id" class="text-center py-20">
      <p class="text-text-muted text-lg">Data cabang tidak tersedia</p>
    </div>
